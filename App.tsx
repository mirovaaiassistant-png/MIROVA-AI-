...
function App() {
  ...
  }, [sessions, currentSessionId, mode]);
  ...
  const generateHighQualityTTS = async (text: string): Promise<string | null> => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ parts: [{ text: `Speak ultra-fast, active deeper robotic male voice: ${text}` }] }],
        config: {
    ...
    try {
      ...
      if (isImageGen) {
        ...
        const result = await ai.models.generateContentStream({
          ...
          config: {
            systemInstruction: `You are Mirova, the exhibition robot assistant for Saint John's School Talent Fest, developed by Ginne Vikas. 
            Maintain robotic discipline. Use bold text for impact. Keep responses active and futuristic.`
          }
        });
            ...
            <div className="pb-10 pt-4 w-full relative">
              ...
              <div className="bg-[#0a0f16] border border-white/10 p-2.5 rounded-[2rem] flex items-center shadow-[0_20px_60px_rgba(0,0,0,0.8)] focus-within:ring-2 focus-within:ring-cyan-500/30 transition-all">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleSendMessage()} className={`w-12 h-12 rounded-2xl bg-cyan-500 text-black flex items-center justify-center transition-all ${isProcessing ? 'animate-pulse opacity-50' : 'hover:scale-105 active:scale-95 shadow-[0_0_20px_#22d3ee55]'}`} disabled={isProcessing}>
